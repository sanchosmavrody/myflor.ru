...
echo "<fieldset style=\"border-style:solid; border-width:1; border-color:black;\"><legend> <span style=\"font-size: 10px; font-family: Verdana\">{$title}</span> </legend>{$message}</fieldset>";
	...
	echo "if(document.addnews.message.value == '' || document.addnews.title.value == ''){ DLEalert('$lang[vote_alert]', '$lang[p_info]'); }
        ...
        dd=window.open('','snd','height=400,width=680,resizable=1,scrollbars=1')
	...
	echo "if(document.addnews.message.value == '' || document.addnews.title.value == ''){ DLEalert('$lang[vote_alert]', '$lang[p_info]'); }
        ...
        dd=window.open('','prv','height=350,width=600,resizable=1,scrollbars=1')
...
HTML;
	...
	if( $_REQUEST['editor'] == "wysiwyg" ) {
		...
		echo "<div class=\"editor-panel\"><div class=\"shadow-depth1\">{$bb_code}<textarea dir=\"auto\" class=\"editor\" style=\"width:100%;height:300px;\" onfocus=\"setFieldName(this.name)\" name=\"message\" id=\"message\" ></textarea></div></div><script>var selField  = \"message\";</script>";
	}
